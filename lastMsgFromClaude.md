commit 13978290dd1c462f4b15b05c6625849e0f53c4a3



⏺ The tool call flow looks correct. Here's a comprehensive summary:
  ...
  │  - Calls orchestrator.process(message)                      │
  └─────────────────────────────────────────────────────────────┘
      │
      ▼
  ┌─────────────────────────────────────────────────────────────┐
  │  orchestrator/router.py: Orchestrator.process()             │
  │  1. route() - LLM decides: tfsa | rrsp | fund_facts         │
  │  2. Gets appropriate tool from self.tools dict              │
  │  3. tool.invoke(query, generate_chart, chart_type)          │
  ...
  │  1. Gets tool-specific FAISS index via settings             │
  │  2. search_client.search(query) → retrieves documents       │
  │  3. build_context() → formats docs for LLM                  │
  │  4. generate_content() → Gemini RAG response                │
  │  5. Optional: generate chart via matplotlib                 │
  │  6. Returns ToolResponse with reply, sources, chart         │
  └─────────────────────────────────────────────────────────────┘
      │
      ▼
  ┌─────────────────────────────────────────────────────────────┐
  │  tools/local_search.py: LocalSearchClient                   │
  │  - Loads FAISS index from tool-specific path                │
  │  - Uses GoogleGenerativeAIEmbeddings for similarity search  │
  │  - Returns SearchResult[] with content, pdf_name, page      │
  └─────────────────────────────────────────────────────────────┘
